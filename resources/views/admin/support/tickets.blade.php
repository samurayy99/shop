...
<div class="content-body">
    ...
    <div class="card">
        <div class="card-body">
            <h6><i class="fal fa-exclamation-triangle"></i> Offene Tickets ({{ App\Models\Ticket::countOpenTickets() }})
            </h6>
            ...
            <table id="open" class="datatables-basic table dataTable no-footer dtr-column" style="width:100%">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ $ticket->id }}</td>
                        <td><a href="{{ route('admin.ticket.show', $ticket->id) }}"
                                class="primary-color">@if($ticket->ticket_title == null) {{ __('Kein Betreff angegeben')
                                }} @else {{ $ticket->ticket_title }} @endif</a></td>
                        <td><span class="badge bg-warning">{{ $ticket->ticket_department }}</span></td>
                        <td>{{ $ticket->order_id != null ? "#{$ticket->order_id}" : '-' }}</td>
                        <td><a href="{{ route('admin.user.edit', $ticket->user_id) }}">{{
                                App\Models\User::getUserById($ticket->user_id)->username }}</a></td>
                        <td>{{ date('d-m-y H:s', strtotime($ticket->created_at)) }}</td>
                        <td>
                            <a href="{{ route('admin.ticket.show', $ticket->id) }}" class="primary-color"><i
                                    class="fal fa-eye"></i></a>
                            <a href="{{ route('admin.ticket.close_open', $ticket->id) }}" class="text-warning"><i
                                    class="fal fa-lock-open"></i></i></a>
