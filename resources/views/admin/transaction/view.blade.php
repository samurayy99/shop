...
<div class="content-body">
    ...
    <div class="card">
        <div class="card-body">
            <h6><i class="fab fa-bitcoin"></i> Alle Transaktionen ({{ App\Models\UserTransaction::countTransactions()
                }})</h6>
            <table id="user" class="datatables-basic table dataTable no-footer dtr-column" style="width:100%">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $transaction->confirmations }}/{{ App\Models\Settings::get('shop.btc_confirms_needed', 1)
                            }}</td>
                        <td>
                            <a class="primary-color" href="{{ route('admin.user.edit', $transaction->user_id) }}">{{
                                App\Models\User::getUsernameById($transaction->user_id) }}</a>
                        </td>
                        <td>
                            {{ date('d.m.Y H:i', strtotime($transaction->created_at)) }}
                        </td>
                        <td>
                            <a href="https://www.blockchain.com/btc/tx/{{ $transaction->txid }}"
                                class="primary-color">auf Blockchain ansehen</a>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
        ...
        <form class="form form-vertical" action="{{ route('admin.transactions.deepsearch') }}" method="GET">
            ...
            <div class="card shadow-none bg-transparent border-success">
                <div class="card-body">
                    ...
                    <p><b>Betrag (BTC):</b> {{ $searchResult["details"][0]["amount"] }} (~{{
                        App\Custom\BitcoinHandler::convertBtc($searchResult["details"][0]["amount"]) }} EUR) mit {{
                        $searchResult["confirmations"] }} Bestätigungen</p>
            ...
            <div class="row">
                ...
                <div class="col-12">
                    <div class="mb-1">
                        ...
                        <div class="input-group input-group-merge">
                            <input type="text" id="s" class="form-control" name="s" autocomplete="off" placeholer="..."
                                required>
                        </div>
            ...
            <div class="col-12">
                <button type="submit"
                    class="btn btn-primary me-1 waves-effect waves-float waves-light">Absenden</button>
            </div>
    </div>
    </form>
</div>
