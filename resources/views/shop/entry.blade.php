...
<div class="content-body">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="multi-range-price">
                            ...
                            <a href="{{ route('shop.cat.show', $category->slug) }}"
                                class="btn btn-warning w-100 btn-sm btn-block waves-effect waves-float waves-light mb-05">
                                @if(isset($selectedCategory) && $selectedCategory->id == $category->id) <i
                                    class="fal fa-chevron-double-right"></i> @endif
                                <i class="fal fa-star"></i> {{ $category->name }}
                            </a>
                            @else
                            <a href="{{ route('shop.cat.show', $category->slug) }}"
                                class="btn btn-secondary w-100 btn-sm btn-block waves-effect waves-float waves-light mb-05">
                                @if(isset($selectedCategory) && $selectedCategory->id == $category->id) <i
                                    class="fal fa-chevron-double-right"></i> @endif
                                {{ $category->name }}
                            </a>
    ...
    <div class="row match-height">
        ...
        @empty
        <div class="col">
            <div class="alert alert-warning p-1"><i class="fal fa-exclamation-triangle"></i> {{ __('Es wurden keine
                Produkte in dieser Kategorie gefunden') }}</div>
        </div>
